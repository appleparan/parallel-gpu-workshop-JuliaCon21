...
It works, but the "naive" _Picard_ iteration count seems to be pretty high (`niter>800`). A efficient way to circumvent this is to add "damping" (`damp`) to the (psuedo-time) rate-of-change `dH/dtau`, analogous to add friction enabling faster convergence \[[4][Frankel50]\]
```md
dH/dtau = ResH + damp * (dHd/tau)_prev
```
where `(dHd/tau)_prev` is the `dH/dtau` value from the previous pseudo-time iteration.
